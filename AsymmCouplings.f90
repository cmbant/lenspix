...
!The cut it taken from a text file of alms, cut_Alm_file, e.g. as converted from HEALPIX output
 ...
 subroutine HealpixAlm2Vector(Alm, v, lmax, polix)
  ...
  if (polix==1) then
   ...
   do l=2, lmax 
         v(idx(2,l,0))=real(Alm%TEB(polix,l,0))  
       end do
     
       do m=1, lmax
        do l=max(2,m), lmax 
            v(idx(2,l,m))=root2*real(Alm%TEB(polix,l,m))
            v(idx(2,l,-m))=root2*aimag(Alm%TEB(polix,l,m))
        end do
       end do 
 ...
 subroutine HealpixVector2Alm(v,Alm, lmax)
  Type (HealpixAlm) :: Alm
  integer, intent(in) :: lmax
  real(sp) :: v(:)
  integer l,m

  call HealpixAlm_Init(Alm,lmax,1)  
  
   do l=0, lmax 
    Alm%TEB(1,l,0) = v(idx(0,l,0))  
   end do
  
   do m=1, lmax
    do l=m, lmax 
     Alm%TEB(1,l,m) = cmplx(v(idx(0,l,m)),v(idx(0,l,-m)))/root2
    end do
  end do 
 ...
 subroutine CutSkyAsymm_GetCoupling(M,WinA, WinAP,lmax, plusonly, almaxr)
  Type (HealpixAlm) :: WinA, WinAP
...
!Have to go to 2*lmax
  ...
  if (asymm_pol) allocate(WlmP(0:lmaxcut,0:lmaxcut))  
  do l1=0,lmaxcut
   Wlm(l1,0:l1) = WinA%TEB(1,l1,0:l1)*sqrt((2*l1+1)/fourpi)
   if (asymm_pol)   WlmP(l1,0:l1) = WinAP%TEB(1,l1,0:l1)*sqrt((2*l1+1)/fourpi)
  end do
 ...
 if (asymm_pol) then
  ...
  do l1=0, lmaxl
   ...
   if (asymm_pol .and. l1>=2 .and. l2>=2) then
...
!we are calculating W(l1,m1, l2,m2)
      ...
      if (.not. asymm_pol .or. l1<2 .or. l2<2) cycle
 ...
 subroutine CutSkyAsymm_GetModeMatrixFromMapAlm(A, AP, Proj, ProjPol,lmax, MINSUPPORT, MapModes, MapA)
  ...
  if (.not. EBOnly) then
   ...
   !Diagonalize (W_+ W_-, -W_- W_+) by diagonalizing hermitian W_+ + iW_i
    m = size(Asymm%WPAsymm,DIM=1)
    n = size(Asymm%WPAsymm,DIM=2)
    if (n/=m) call MpiStop('not checked m/=n')
    allocate(diag(m))
    
    allocate(WComp(m,n))
    WComp = Asymm%WPAsymm - cmplx(0,1)*Asymm%WMAsymm  !W_+ + iW_-
    ...
    nfound = n
    call Matrix_CDiagonalize_Partial(WComp, diag, n, MINSUPPORT,1e100_dp, nfound)
    !Complex version. Does m = U diag U^dag, returning U in WComp
 
    print *,'n pol modes = ', nfound, ' of ', n
    print *, 'min/max diag = ', minval(diag(1:nfound)),maxval(diag(1:nfound))
    print *,'sum diag^2 = ', sum(diag(1:nfound)**2)

    allocate(ProjPol%Wcomp(n,nfound))
    ProjPol%nr = nfound
    ProjPol%nl = n
    ProjPol%WComp = WComp(:,1:nfound)
       
    allocate(ProjPol%RootDiag(nfound))
    ProjPol%RootDiag = sqrt(diag(1:nfound))
    deallocate(diag)
